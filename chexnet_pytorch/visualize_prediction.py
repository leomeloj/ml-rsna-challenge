...
def calc_cam(x, label, model):
    ...
    # find index for label; this corresponds to index from output of netdensenet_last_laye
    label_index = next(
...
def calc_cam_pneumonia(x, model):
    """
    function to generate a class activation map corresponding to a torch image tensor
    ...
    #normalize by pneumonia probability
    cam = cam/0.012
...
def load_rsna_data_metadata(
    ...
    model = densenet.densenet121(False,**kwargs)
...
def generate_attention(dataloader, model):
    
    for inp in range(len(dataloader.dataset)):
        inputs, labels, filename = next(dataloader)
        #print(labels)
        raw_cam = calc_cam_pneumonia(inputs, model)
        image_data = [[int(i*1000) for i in nested] for nested in raw_cam.squeeze()]                
        image_out = Image.new('L',(len(raw_cam.squeeze()),len(raw_cam.squeeze()[0])))
        image_out.putdata(image_data)
        image_out.save("hmaps/"+filename[0])
        '''
        hmap = sns.heatmap(raw_cam.squeeze(),
            cmap = 'viridis',
            alpha = 0.3, # whole heatmap is translucent
        ...
        print(filename)
        figure.savefig("hmaps/"+filename[0], dpi=400)
        '''        

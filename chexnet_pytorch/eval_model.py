...
def make_pred_multilabel_meta(data_transforms, model, PATH_TO_IMAGES, PATH_TO_META):
    """
    Gives predictions for test fold and calculates AUCs using previously trained model
    ...
    for i, data in enumerate(dataloader):
        ...
        inputs, labels, meta = Variable(inputs.cuda()), Variable(labels.cuda()).float(), Variable(meta.cuda()).float()
...
def predict_test(data_transforms, model, PATH_TO_IMAGES):
    """
    Gives predictions for test fold and calculates AUCs using previously trained model
    ...
    pred_df.to_csv("test/preds.csv", index=False)
    return pred_df
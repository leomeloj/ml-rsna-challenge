...
Modified by Phillip Cheng MD MS for the RSNA Pneumonia Detection Challenge 
"""
...
from keras_retinanet.callbacks.eval_rsna import Evaluate
...
def create_models(backbone_retinanet, num_classes, weights, multi_gpu=0, freeze_backbone=False):
    ...
    training_model.compile(
        ...
        optimizer=keras.optimizers.sgd(lr=1e-2, momentum=0.9, decay=.0001, nesterov=True, clipnorm=1) #, clipnorm=0.001)
    )
...
def parse_args(args):
    ...
    parser.add_argument('--snapshot-path',   help='Path to store snapshots of models during training', default=default_snapshot_dir)
    parser.add_argument('--tensorboard-dir', help='Log directory for Tensorboard output', default='')
